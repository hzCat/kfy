...
Page({
  ...
  chooseMoney(e) {
    ...
    that.getVipInfo(that.data.vipScope, e.target.dataset.money, that.data.header);
  },
                  ...
                  pay.chargeback(orderId, header)
                    .then(function (res) {
                      ...
                      if (obj.tradeStatus == "SUCCESS") {
                        ...
                        // 关闭提示框
                        wx.hideLoading();
                        util.jump("redirect", `/pages/chargeSuccFail/chargeSuccFail?isBuy=false&isSucc=true&money=${money}&orderNo=${orderNo}&gift=${gift}&after=${after}`)
                        // FAIL ERROR订单(isSucc,orderId)
                      } else if (obj.tradeStatus == "FAILED" || obj.data.TradeStatus == "ERROR") {
                        wx.hideLoading();
                        util.jump("nav", `/pages/chargeSuccFail/chargeSuccFail?isBuy=false&isSucc=false&orderNo=${orderNo}`);
                        //UNKNOWN状态订单
                      } else if (obj.tradeStatus == "UNKNOWN") {
                        // 第二次查询,3s
                        setTimeout(function () {
                          pay.chargeback(res.data.data.orderId, header)
                            .then(function (res) {
                              let obj = res.data;
                              let money = obj.tradeResponse.rechargeAmt;
                              let orderNo = obj.tradeResponse.orderNo;
  ...
  // 获取协议文档
  getDoc() {
    let downloadUrl = "http://192.168.1.146:11811/group1/M00/00/01/wKgBkloeYFyAdaptAABP-yHX5mA65_big.docx";
    // 下载
    wx.downloadFile({
      url: downloadUrl,
      success: function (res) {
