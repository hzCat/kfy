  ...
  onLoad: function (options) {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        } else if (options.by == 'orderlist') { //订单页面进入
                    ...
                    pay.payback(that.data.orderData.id, that.data.header)
                      .then(function (res) {
                        ...
                        if (obj.tradeStatus == "SUCCESS") {
                          wx.hideLoading();
                          util.jump("redirect", `/pages/paysuccess/paysuccess?by=wechat&isSucc=true&money=${money}&orderId=${orderNo}`)
                          // FAIL ERROR订单(by,isSucc,orderId)
                        } else if (obj.tradeStatus == "FAILED" || obj.tradeStatus == "ERROR") {
                          wx.hideLoading()
                          util.jump("nav", `/pages/paysuccess/paysuccess?by=wechat&isSucc=false&orderId=${orderNo}`);
                          //UNKNOWN状态订单
                          ...
                          setTimeout(function () {
                            ...
                            pay.payback(that.data.orderData.id, that.data.header)
                              .then(function (res) {
                                ...
                                if (obj.tradeStatus == "SUCCESS") {
                                  wx.hideLoading()
                                  util.jump("redirect", `/pages/paysuccess/paysuccess?by=wechat&money=${money}&orderId=${orderNo}`)
                                  // FAIL ERROR订单(by,isSucc,orderId)
                                } else {
                                  wx.hideLoading()
                                  util.jump("nav", `/pages/paysuccess/paysuccess?by=wechat&isSucc=false&orderId=${orderNo}`);
      ...
      pay.vpay(data, header)
        .then(function (res) {
          ...
          if (result == true && code == 200 && orderNo != null) {
            ...
            util.jump("redirect", `/pages/paysuccess/paysuccess?by=card&isSucc=true&money=${money}&orderId=${orderNo}`);
          } else {
            util.jump("nav", `/pages/paysuccess/paysuccess?by=card&isSucc=false&code=${code}&orderId=${orderNo}`);
          }
        })
        .catch(function (err) {
          console.log(err);
      ...
      pay.vpay(data, header)
        .then(function (res) {
          ...
          if (result == true && code == 200 && orderNo != null) {
            ...
            util.jump("nav", `/pages/paysuccess/paysuccess?by=card&isSucc=false&code=${code}`)
          }
        })
        .catch(function (err) {
          // 遮罩
